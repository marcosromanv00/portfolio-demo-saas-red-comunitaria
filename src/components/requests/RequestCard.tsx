    ...
    >
      ...
      <div>
        ...
        <div className="mt-3 flex flex-col items gap-2 opacity-80">
          {request.category && (
            <span className="badge">🏷️ {request.category}</span>
          )}
          {request.location && (
            <span className="badge">📍 {request.location}</span>
          )}
          <span className="badge badge-status">⚙️ {request.status}</span>
        </div>
      </div>
      ...
      <div className="-mt-4 flex justify-end-safe text-sm">
        <div className={ACTION_GROUP}>
          ...
          <button
            ...
            className={
              ACTION_BUTTON + " " + ACTION_BUTTON_DANGER + " rounded-r-sm"
            }
          >
